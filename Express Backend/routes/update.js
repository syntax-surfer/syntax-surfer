...
router.put("/", async (req, res) => {
  ...
  if (typeof requestBody !== "object" || Array.isArray(requestBody)) {
    return res
      .status(400)
      .json({ error: "Invalid JSON format. Request body must be an object." });
  }
  ...
  for (const prop of requiredProperties) {
    //Ensure Request has the correct properties.
  ...
  try {
    ...
    if (requestBody.origin === "embed") { //If it was from Embed service start job
